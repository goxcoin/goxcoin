...
uint256 hashGenesisBlock("0x3d06216b0d0f0a32b3ff4f0d5bdd5d1745b41758fca926e4da7c84d9cf8540fd");
...
const string strMessageMagic = "GoxCoin Signed Message:\n";
...
{
    // GoxCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 5 * 60; // GoxCoin: 5 minutes
static const int64 nTargetSpacing = 60; // GoxCoin: each minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GoxCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GoxCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // GoxCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "MtGox 2014/02/07 - Incompetence causes losses amounting to millions of dollars.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391796001;
        block.nBits    = 0x1e3fffff;
        block.nNonce   = 82982038;
        ...
        assert(block.hashMerkleRoot == uint256("0x86ab8e8da807a0642990a9b2f5a15b5e3091ee592f3a2ce8e6b52d5e272887e0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // GoxCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GoxCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoxCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoxCoinMiner(CWallet *pwallet)
{
    printf("GoxCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("goxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GoxCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoxCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoxCoinMiner, pwallet));
}
